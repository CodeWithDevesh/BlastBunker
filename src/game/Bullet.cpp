...
void Bullet::Update() {
  ...
  if (bodyPos.x < 0 || bodyPos.x > screenWidth || bodyPos.y < 0 ||
      bodyPos.y > screenHeight) {
    Destroy();
  }
}
...
void Bullet::Draw() {
  ...
  DrawTexturePro(m_bodyTexture, frameRec,
                 {bodyPos.x, bodyPos.y, (float)frameWidth, (float)frameHeight},
                 {(float)frameWidth / 2, (float)frameHeight / 2}, bodyAngle,
                 WHITE);
}
