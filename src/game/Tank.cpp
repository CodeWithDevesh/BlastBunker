...
Tank::Tank(b2Vec2 pos, TankType type, TankColor color) {
  ...
  m_bodyTexture =
      Globals::GetAssetManager()->getTexture(TEXTURE_TANK_GREEN_BODY);
  m_turretTexture =
      Globals::GetAssetManager()->getTexture(TEXTURE_TANK_GREEN_TURRET);
...
void Tank::Update() {
  ...
  if (moving) {
    ...
    if (fabsf(angleDiff) < angleThreshold) {
      ...
      b2Body_SetAngularVelocity(m_bodyId,
                                angleDiff > 0 ? turnSpeed : -turnSpeed);
    }
  }

  b2Vec2 currentRightNormal =
      b2Body_GetWorldVector(m_bodyId, {1, 0}); // Right direction of tank
  b2Vec2 lateralVelocity =
      b2Dot(currentRightNormal, b2Body_GetLinearVelocity(m_bodyId)) *
      currentRightNormal;

  float driftControl = 0.2f;

  b2Vec2 impulse = -lateralVelocity * b2Body_GetMass(m_bodyId) * driftControl;
  b2Body_ApplyLinearImpulse(m_bodyId, impulse,
                            b2Body_GetWorldCenterOfMass(m_bodyId), true);
...
void Tank::Draw() {
  ...
  DrawTexturePro(
      m_bodyTexture, bodyFrameRec,
      {bodyPos.x, bodyPos.y, (float)bodyFrameWidth, (float)bodyFrameHeight},
      {(float)bodyFrameWidth / 2, (float)bodyFrameHeight / 2}, bodyAngle,
      WHITE);
  ...
  DrawRectangle(healthBarX, healthBarY, healthBarWidth * healthPercentage,
                healthBarHeight, healthBarColor);

  // Draw the turret
  DrawTexturePro(
      m_turretTexture, turretFrameRec,
      {bodyPos.x, bodyPos.y, (float)turretFrameWidth, (float)turretFrameHeight},
      {(float)turretFrameWidth / 2, (float)turretFrameHeight - turretOffset},
      turretAngle, WHITE);
}
...
void Tank::DrawDebug() {
  ...
  DrawRectanglePro({pos.x, screenHeight - pos.y, (float)bodyFrameWidth,
                    (float)bodyFrameHeight},
                   {(float)bodyFrameWidth / 2, (float)bodyFrameHeight / 2},
                   -bodyAngle, BLUE);
  DrawRectanglePro(
      {pos.x, pos.y, (float)bodyFrameWidth, (float)bodyFrameHeight},
      {(float)bodyFrameWidth / 2, (float)bodyFrameHeight / 2}, bodyAngle, RED);
}
